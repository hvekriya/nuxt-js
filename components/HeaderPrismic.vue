<template>
  <header class="site-header">
    ...
    <nav>
      <ul class="nav">
        <li v-for="(nav, id) in $store.state.menu.nav" :key="id">
          <prismic-link :field="nav.primary.link">{{ $prismic.asText(nav.primary.label) }}</prismic-link>
          <ul v-if="nav.items.length != 0">
            <li v-for="(item, id) in nav.items" :key="id">
              <prismic-link
